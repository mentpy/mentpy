...
TAKEN FROM: https://github.com/XanaduAI/flamingpy/blob/main/flamingpy/utils/linalg.py
"""
...
def lc_constraint_system(G, H):
    ...
    for j in range(n):
        ...
        # C block
        C = np.array(
            [[G[i, j] * H[i, k] for i in range(n)] for k in range(n)], dtype=int
        )
        # D block
...
def are_lc_equivalent(graph1, graph2, clifford_form="tensor"):
    ...
    Args:
        graph1: the initial graph to check Clifford equivalence with.
        graph2: the graph to check Clifford equivalence against.
    ...
    G = nx.adjacency_matrix(graph1.graph)
    H = nx.adjacency_matrix(graph2.graph)
